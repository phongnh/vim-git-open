...
56. **`g:vim_git_open_browser_command` init order** — set from OS default first (`open` macOS, `start` Windows, `xdg-open` otherwise — no empty fallback); then override with `$BROWSER` if set. Applies at plugin load time in all three entry points (`plugin/git_open.vim`, `vim9/plugin/git_open.vim`, `lua/git_open.lua` via `M.setup`). `opts.browser_command` in Lua is applied first, then OS default fills any gap, then `$BROWSER` always wins.
57. **`OpenBrowser {url}` user command** — opens a URL directly in the browser. Single public function in each implementation: `git_open#OpenBrowser` (legacy), `export def OpenBrowser` (Vim9), `M.open_browser` (Lua). No private wrapper. Legacy `s:OpenOrCopy` calls `git_open#OpenBrowser`; Vim9 `OpenOrCopy` calls `OpenBrowser`; Lua `open_or_copy` calls `M.open_browser`. Windows uses `start "" <url>` (shell), not `!start` (ex-command).
58. **`redraw!` only after `system()`** — `CopyToClipboard`/`copy_to_clipboard` does not call `system()`, so no `redraw!` needed. Only `OpenBrowser`/`open_browser` needs `redraw!` (after the subprocess call).
59. **Lua messages use `vim.api.nvim_echo`** — `print()` writes to `:messages` history; `vim.api.nvim_echo({{msg}}, false, {})` displays without adding to history. All informational echoes (`Opened:`, `Copied:`, `git-open: remote ...`) use `nvim_echo` with `history=false`.
60. **Non-capturing groups `\%(...\)`** — use in VimL/Vim9 wherever a `matchlist` group is structural (its value is discarded). SSH pattern: `\%(git@\|ssh://git@\)` (group 1 unused → index shifts: domain `[1]`, path `[2]`). HTTPS pattern: `\%(https\?://\)` and `\%(\.git\)\?` (groups 1 and 4 unused → domain `[1]`, path `[2]`). Provider `matchlist` patterns keep capturing groups because `match[1]` is the extracted PR/MR number.
